...
class SyntheticHeadPoseGeneration:
    ...
    def __call__(self, num_frames, lower_upper_lip_expressions, source_info):
        t = torch.linspace(0, int(num_frames / self.fps), num_frames).to(self.device)
        if t.shape[0] < num_frames:
            t = torch.cat([t, t[-1].unsqueeze(0).repeat(1, num_frames - t.shape[0])], dim=-1)
        else:
            t = t[:num_frames]

        eased_t = ease_in_out_sine(t).to(self.device)
        ...
        for pose_name in pose_hash.keys():
            value = self.generate_head_moves(t=t,
                                             ...
                                             eased_t=eased_t)
            # Ensure both value and source_info[pose_name] are on the same device
            value = (value * weight_list) + source_info[pose_name][0, 0].to(self.device).detach()
    ...
    def smooth_weight_list(self, weight_list):
        ...
        kernel = torch.ones(self.window_size, device=self.device) / self.window_size
    ...
    def get_lip_based_weight_list(self, lower_upper_lip_expressions):
        ...
        normalized_lip_based_exp = minmax_normalization(lower_upper_lip_expressions).to(self.device)
